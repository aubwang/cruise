...
class CruiseSessionTest(unittest.TestCase):
    ...
    def test_package_sync_creates_neutral_installable_skills(self) -> None:
        ...
        self.assertIn("cruise-setup check", setup_text)
        for skill_text in [setup_text, handoff_text, zoom_out_text]:
    ...
    def test_planning_warning_when_context_map_missing(self) -> None:
        ...
        self.assertIn("`## Agent Context Map` block is missing", result.stdout)
        self.assertIn("ROADMAP.md", result.stdout)
    ...
    def test_planning_warning_silent_with_legacy_heading(self) -> None:
        (self.root / "ROADMAP.md").write_text("# Product Roadmap\n", encoding="utf-8")
        self.run_cli("cruise-setup", "apply")
        self.run_cli("cruise-setup", "instructions", "AGENTS.md")
        agents = self.root / "AGENTS.md"
    ...
    def test_apply_creates_gitignore_with_cruise_block(self) -> None:
        ...
        self.assertIn(".cruise/", gitignore)
