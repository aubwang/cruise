...
`apply` does **not** create `AGENTS.md`, `CLAUDE.md`, `ROADMAP.md`, `docs/PLAN.md`, or any other repo-owned file.
...
If you find Cruise operational guidance outside the managed block (e.g. "read `.cruise/protocol.md` at session start" in freeform text), propose removing or trimming it — those instructions belong only in the managed protocol block.
...
- `AGENTS.md` and `CLAUDE.md` at the repo root — is there already an `## Agent Context Map` section? (Also check for the legacy heading `## Agent skills`.)
...
## 5. Edit the `## Agent Context Map` block

Show the user a draft and let them edit before writing. The context map describes documentation ownership and layout only — it tells agents where durable repo context lives, not how to operate Cruise.

If a legacy `## Agent skills` block exists, rename the heading to `## Agent Context Map` in place rather than appending a duplicate. If a block already exists under either heading, update its contents in-place. Don't overwrite user edits to surrounding sections.
...
Accepted shipped ADRs live in `docs/adr/`. Provisional planning notes live in `.cruise/spec.md` under `## Provisional decisions`.

### Planning files

[list each repo-owned planning file that provides durable product context, with a one-line description of what it contains. Every detected planning file must appear here by name — `cruise-setup check` warns until they do.]
```
...
The setup report's `## Planning files` section lists any detected planning docs (`ROADMAP.md`, `docs/PLAN.md`, `PLAN.md`, etc.). For each one, ask the user:
...
3. **What should the durable split be?** Confirm and record in the `## Agent Context Map` block from step 5. The default split is:
...
Tell the user the setup is complete, name the durable split that was confirmed in step 6, and which engineering skills will now read from `CONTEXT.md` or `CONTEXT-MAP.md`, `docs/adr/`, and `.cruise/spec.md`. Mention they can edit the `## Agent Context Map` block later if they change the domain-doc layout.
