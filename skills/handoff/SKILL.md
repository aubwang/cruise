...
The handoff command reads `.cruise/plan.md` and bakes its `## Current objective` and `## Current slice` into `HANDOFF.md`, `.cruise/next.md`, and `.cruise/sessions/`. Stale plan.md means a stale handoff that misleads the next agent.
...
After the command runs, report the paths for `.cruise/next.md`, `.cruise/sessions/latest.md`, and `HANDOFF.md`. If the command printed a `warning:` line, surface it to the user and refresh plan.md before declaring the handoff done.
